...
'@
            ...
            $res | Should -BeExactly "$(Get-PromptConnectionInfo)$path [main +1 ~0 -0 | +0 ~1 -1 !]> "
        }
...
'@
            ...
            Should -Invoke -ModuleName posh-git -CommandName git -Exactly 1
            $path = GetHomeRelPath $PSScriptRoot
            $res | Should -BeExactly "$(Get-PromptConnectionInfo)$path !! [main]> "
        }
...
'@
            ...
            Should -Invoke -ModuleName posh-git -CommandName git -Exactly 1
            $path = GetHomeRelPath $PSScriptRoot
            $res | Should -BeExactly "$(Get-PromptConnectionInfo)$path - 42 [main]> "
        }
...
'@
            ...
            Should -Invoke -ModuleName posh-git -CommandName git -Exactly 1
            $path = GetHomeRelPath $PSScriptRoot
            $res | Should -BeExactly "$(Get-PromptConnectionInfo)$path ${csi}93m[${csi}39m${csi}96mmain${csi}39m${csi}32m +1${csi}39m${csi}32m ~0${csi}39m${csi}32m -0${csi}39m${csi}93m |${csi}39m${csi}31m +0${csi}39m${csi}31m ~1${csi}39m${csi}31m -1${csi}39m${csi}31m !${csi}39m${csi}93m]${csi}39m> "
        }
...
'@
            ...
            Should -Invoke -ModuleName posh-git -CommandName git -Exactly 1
            $path = GetHomeRelPath $PSScriptRoot
            $res | Should -BeExactly "$(Get-PromptConnectionInfo)$path${csi}107m !! ${csi}49m${csi}93m[${csi}39m${csi}96mmain${csi}39m${csi}93m]${csi}39m> "
        }
        It 'Returns the expected prompt string with expanded PathStatusSeparator' {
            Mock -ModuleName posh-git -CommandName git {
                $OFS = " "
...
'@
            ...
            Should -Invoke -ModuleName posh-git -CommandName git -Exactly 1
            $path = GetHomeRelPath $PSScriptRoot
            $res | Should -BeExactly "$(Get-PromptConnectionInfo)$path${csi}107m [$(hostname)] ${csi}49m${csi}93m[${csi}39m${csi}96mmain${csi}39m${csi}93m]${csi}39m> "
        }
    }
}
...
Describe 'Default Prompt WindowTitle Tests' -Skip:$SkipWindowTitleTests {
    BeforeAll {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseDeclaredVarsMoreThanAssigments', '')]
        $repoAdminRegex = '^Admin: posh-git \[main\] - PowerShell \d+\.\d+ (\d\d-bit )?\(\d+\)$'
        [System.Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseDeclaredVarsMoreThanAssigments', '')]
        $repoRegex = '^posh-git \[main\] - PowerShell \d+\.\d+ (\d\d-bit )?\(\d+\)$'
        [System.Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseDeclaredVarsMoreThanAssigments', '')]
...
'@
        ...
        It 'Custom GitPromptSettings.WindowTitle scriptblock sets the expected Window title text' {
            ...
            $title = $Host.UI.RawUI.WindowTitle
            if (& $module { $IsAdmin }) {
                $title | Should -Match '^daboss: poshgit == posh-git / main$'
            }
            else {
                $title | Should -Match '^loser: poshgit == posh-git / main$'
            }
        }
