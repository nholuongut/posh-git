...
Describe 'Proxy Function Expansion Tests' {
    ...
    Context 'Proxy Function Parameter Replacement Tests' {
        ...
        It 'Replaces parameter in $args' {
            $result = & $module Expand-GitProxyFunction 'Invoke-GitFunction main'
            $result | Should -Be 'git checkout main'
        }
        It 'Replaces short parameter in $args' {
            $result = & $module Expand-GitProxyFunction 'Invoke-GitFunction -b main'
            $result | Should -Be 'git checkout -b main'
        }
        It 'Replaces long parameter in $args' {
            $result = & $module Expand-GitProxyFunction 'Invoke-GitFunction --detach main'
            $result | Should -Be 'git checkout --detach main'
        }
        It 'Replaces mixed parameters in $args' {
            $result = & $module Expand-GitProxyFunction 'Invoke-GitFunction -q -f -m --detach main'
            $result | Should -Be 'git checkout -q -f -m --detach main'
        }
    }
