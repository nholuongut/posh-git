...
.DESCRIPTION
    ...
    * main
    ...
    IMPORTANT: Be careful using this command. Even though by default this command deletes only merged branches, most, if not all, of your historical branches have been merged. But that doesn't mean you want to delete them. That is why this command excludes `develop` and `main` by default. But you may use different names e.g. `development` or have other historical branches you don't want to delete. In these cases, you can either:

    * Specify a narrower branch name wildcard such as "user/$env:USERNAME/*".
    * Specify an updated ExcludeParameter e.g. '(^\*)|(^. (develop|main|v\d+)$)' which adds any branch matching the pattern 'v\d+' to the exclusion list.
...
.EXAMPLE
    PS> Remove-GitBranch -Pattern 'user/(nholuongut|hillr)/.*'
    Deletes the merged branches that match the specified regular expression.
.EXAMPLE
    PS> Remove-GitBranch -Name * -ExcludePattern '(^\*)|(^. (develop|main|v\d+)$)'
    Deletes merged branches except the current branch, develop, main and branches that also match the pattern 'v\d+' e.g. v1, v1.0, v1.x. BE VERY CAREFUL SPECIYING SUCH A BROAD BRANCH NAME WILDCARD!
.EXAMPLE
...
function Remove-GitBranch {
    ...
    param(
        # Specifies a regular expression pattern for the branches that will be deleted. Certain branches are always excluded from deletion e.g. the current branch as well as the develop and main branches. See the ExcludePattern parameter to modify that pattern.
        [Parameter(Position = 0, Mandatory, ParameterSetName = "Wildcard")]
        [ValidateNotNullOrEmpty()]
        [string]
        $Name,

        # Specifies a regular expression for the branches that will be deleted. Certain branches are always excluded from deletion e.g. the current branch as well as the develop and main branches. See the ExcludePattern parameter to modify that pattern.
        [Parameter(Position = 0, Mandatory, ParameterSetName = "Pattern")]
        [ValidateNotNull()]
        [string]
        $Pattern,

        # Specifies a regular expression used to exclude merged branches from being deleted. The default pattern excludes the current branch, develop and main branches.
        [Parameter()]
        [ValidateNotNull()]
        [string]
        $ExcludePattern = '(^\*)|(^. (develop|main)$)',
...
function Update-AllBranches($Upstream = 'main', [switch]$Quiet) {
