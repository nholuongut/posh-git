...
function Setup-Environment {
    Cleanup
    $env:poshGit = join-path (Resolve-Path .\Tests ) nholuongut-posh-git-60be436.zip
...
Describe "Install-Posh-Git" {
    ...
    It "WillOutputVcsStatus" {
        ...
        try{
            ...
            Pop-Location
            $wh.should.be("$pwd\PoshTest [main]")
        }
        catch {
    ...
    It "WillSucceedOnUpdatingFrom040" {
        ...
        try{
            Remove-Item $Profile -Force
            Add-Content $profile -value ". 'C:\tools\poshgit\nholuongut-posh-git-60be436\profile.example.ps1'" -Force
            RunInstall
            mkdir PoshTest
            Push-Location PoshTest
            git init
